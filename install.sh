...
#   curl -fsSL https://recoil.sh/install | sh -s -- --version v0.1.1
#   curl -fsSL https://recoil.sh/install | sh -s -- --bin-dir /opt/bin
#
# Environment equivalents: RECOIL_VERSION, RECOIL_INSTALL_DIR.
# Every download is verified against the release's checksums.txt before install.
# Windows: irm https://recoil.sh/install.ps1 | iex
...
VERSION="${RECOIL_VERSION:-}"
BIN_DIR="${RECOIL_INSTALL_DIR:-}"
...
Environment:
  RECOIL_VERSION       same as --version
  RECOIL_INSTALL_DIR   same as --bin-dir
EOF
}
...
detect_os() {
  ...
  case "$os" in
    ...
    MINGW*|MSYS*|CYGWIN*|Windows_NT)
      die "Windows detected. In PowerShell run: irm https://recoil.sh/install.ps1 | iex  (or grab a zip from https://github.com/$REPO/releases/latest)" ;;
...
detect_arch() {
  ...
  case "$arch" in
    ...
    *) die "unsupported architecture: $arch (recoil ships x86_64 and arm64; build from source with: CGO_CFLAGS=\"-DSQLITE_ENABLE_FTS5\" go install github.com/$REPO@latest)" ;;
  esac
}
...
# Release assets keep the v, e.g. recoil_v0.1.1_linux_x86_64.tar.gz
ASSET="${BINARY}_${VERSION}_${OS}_${ARCH}.tar.gz"
BASE="https://github.com/$REPO/releases/download/$VERSION"

TMP=$(mktemp -d 2>/dev/null || mktemp -d -t recoil)
trap 'rm -rf "$TMP"' EXIT INT TERM

say "installing recoil $VERSION ($OS/$ARCH)"
...
[ -w "$DEST" ] || die "$DEST is not writable; re-run with --bin-dir <dir>, or: sudo sh -c \"\$(curl -fsSL https://recoil.sh/install)\""

# Write beside the target, then rename, so a running recoil is never half-overwritten.
